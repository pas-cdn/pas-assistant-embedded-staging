import { p as Z } from "./chunk-4BX2VUAB-Dry3wvbl.js";
import { I as F } from "./chunk-QZHKN3VN-C-21b-wm.js";
import { _ as h, J as U, I as rr, s as er, h as tr, i as ar, j as nr, m, k as sr, l as or, H as cr, X as ir, P as dr, y as B, Y as hr, Z as lr, $ as $r, a0 as fr } from "./mermaid-VLURNSYL-C7dSNHug.js";
import { p as yr } from "./treemap-KMMF4GRG-CEx6UpjY.js";
var u = {
  NORMAL: 0,
  REVERSE: 1,
  HIGHLIGHT: 2,
...
function A(t) {
  ...
  if (m.debug(s.join(" ")), r.parents && r.parents.length == 2 && r.parents[0] && r.parents[1]) {
    ...
    Y(t, r, e), r.parents[1] && t.push(i.records.commits.get(r.parents[1]));
  } else {
...
var Cr = /* @__PURE__ */ h(function() {
...
}, S = sr(), b = S == null ? void 0 : S.gitGraph, R = 10, I = 40, k = 4, L = 2, O = 8, C = /* @__PURE__ */ new Map(), E = /* @__PURE__ */ new Map(), P = 30, G = /* @__PURE__ */ new Map(), W = [], M = 0, x = "LR", jr = /* @__PURE__ */ h(() => {
...
}, "findClosestParentBT"), Yr = /* @__PURE__ */ h((t, r, a) => {
  let s = a, e = a;
  const n = [];
  t.forEach((c) => {
    const o = r.get(c);
    ...
    o.parents.length ? (s = Dr(o), e = Math.max(s, e)) : n.push(o), Kr(o, s);
  }), s = e, n.forEach((c) => {
    Nr(c, s, a);
  }), t.forEach((c) => {
    const o = r.get(c);
  ...
  return a;
}, "findClosestParentPos"), Dr = /* @__PURE__ */ h((t) => Ar(t) + I, "calculateCommitPosition"), Kr = /* @__PURE__ */ h((t, r) => {
  ...
  if (r.tags.length > 0) {
    ...
    for (const l of r.tags.reverse()) {
      const f = t.insert("polygon"), y = t.append("circle"), d = t.append("text").attr("y", a.y - 16 - n).attr("class", "tag-label").text(l), g = (e = d.node()) == null ? void 0 : e.getBBox();
      if (!g)
    ...
    for (const { tag: l, hole: f, rect: y, yOffset: d } of $) {
      const g = o / 2, p = a.y - 19.2 - d;
      if (y.attr("class", "tag-label-bkg").attr(
        "points",
        `
      ${s - c / 2 - k / 2},${p + L}  
      ${s - c / 2 - k / 2},${p - L}
      ${a.posWithOffset - c / 2 - k},${p - g - L}
      ${a.posWithOffset + c / 2 + k},${p - g - L}
      ${a.posWithOffset + c / 2 + k},${p + g + L}
      ${a.posWithOffset - c / 2 - k},${p + g + L}`
  ...
  let n = x === "TB" || x === "BT" ? P : 0;
  const c = [...r.keys()], o = (b == null ? void 0 : b.parallelCommits) ?? !1, $ = /* @__PURE__ */ h((f, y) => {
    var p, w;
    const d = (p = r.get(f)) == null ? void 0 : p.seq, g = (w = r.get(y)) == null ? void 0 : w.seq;
    return d !== void 0 && g !== void 0 ? d - g : 0;
  }, "sortKeys");
  let l = c.sort($);
  x === "BT" && (o && Yr(l, r, n), l = l.reverse()), l.forEach((f) => {
    var g;
    const y = r.get(f);
    if (!y)
    ...
    const d = Zr(y, n, o);
    if (a) {
      const p = Jr(y), w = y.customType ?? y.type, q = ((g = C.get(y.branch)) == null ? void 0 : g.index) ?? 0;
      _r(s, y, d, p, q, w), Vr(e, y, d, n), Xr(e, y, d, n);
    }
    x === "TB" || x === "BT" ? E.set(y.id, { x: d.x, y: d.posWithOffset }) : E.set(y.id, { x: d.posWithOffset, y: d.y }), n = x === "BT" && o ? n + I : n + I + R, n > M && (M = n);
  });
  ...
  let o = "", $ = "", l = 0, f = 0, y = (g = C.get(a.branch)) == null ? void 0 : g.index;
  a.type === u.MERGE && r.id !== a.parents[0] && (y = (p = C.get(r.branch)) == null ? void 0 : p.index);
  let d;
  if (c) {
    o = "A 10 10, 0, 0, 0,", $ = "A 10 10, 0, 0, 1,", l = 10, f = 10;
    const T = e.y < n.y ? H(e.y, n.y) : H(n.y, e.y), v = e.x < n.x ? H(e.x, n.x) : H(n.x, e.x);
    x === "TB" ? e.x < n.x ? d = `M ${e.x} ${e.y} L ${v - l} ${e.y} ${$} ${v} ${e.y + f} L ${v} ${n.y - l} ${o} ${v + f} ${n.y} L ${n.x} ${n.y}` : (y = (w = C.get(r.branch)) == null ? void 0 : w.index, d = `M ${e.x} ${e.y} L ${v + l} ${e.y} ${o} ${v} ${e.y + f} L ${v} ${n.y - l} ${$} ${v - f} ${n.y} L ${n.x} ${n.y}`) : x === "BT" ? e.x < n.x ? d = `M ${e.x} ${e.y} L ${v - l} ${e.y} ${o} ${v} ${e.y - f} L ${v} ${n.y + l} ${$} ${v + f} ${n.y} L ${n.x} ${n.y}` : (y = (q = C.get(r.branch)) == null ? void 0 : q.index, d = `M ${e.x} ${e.y} L ${v + l} ${e.y} ${$} ${v} ${e.y - f} L ${v} ${n.y + l} ${o} ${v - f} ${n.y} L ${n.x} ${n.y}`) : e.y < n.y ? d = `M ${e.x} ${e.y} L ${e.x} ${T - l} ${o} ${e.x + f} ${T} L ${n.x - l} ${T} ${$} ${n.x} ${T + f} L ${n.x} ${n.y}` : (y = (D = C.get(r.branch)) == null ? void 0 : D.index, d = `M ${e.x} ${e.y} L ${e.x} ${T + l} ${$} ${e.x + f} ${T} L ${n.x - l} ${T} ${o} ${n.x} ${T - f} L ${n.x} ${n.y}`);
  } else
  ...
  t.append("path").attr("d", d).attr("class", "arrow arrow" + y % O);
  ...
  r.forEach((s, e) => {
    ...
    const g = l.getBBox();
    f.attr("class", "branchLabelBkg label" + n).attr("rx", 4).attr("ry", 4).attr("x", -g.width - 4 - ((b == null ? void 0 : b.rotateCommitLabel) === !0 ? 30 : 0)).attr("y", -g.height / 2 + 8).attr("width", g.width + 18).attr("height", g.height + 4), d.attr(
      "transform",
      "translate(" + (-g.width - 14 - ((b == null ? void 0 : b.rotateCommitLabel) === !0 ? 30 : 0)) + ", " + (c - g.height / 2 - 1) + ")"
    ), x === "TB" ? (f.attr("x", c - g.width / 2 - 10).attr("y", 0), d.attr("transform", "translate(" + (c - g.width / 2 - 5) + ", 0)")) : x === "BT" ? (f.attr("x", c - g.width / 2 - 10).attr("y", M), d.attr("transform", "translate(" + (c - g.width / 2 - 5) + ", " + M + ")")) : f.attr("transform", "translate(-19, " + (c - g.height / 2) + ")");
  });
...
`, "id:", r, a), !b)
  ...
  c.forEach((l, f) => {
    var q;
    const y = J(l.name), d = o.append("g"), g = d.insert("g").attr("class", "branchLabel"), p = g.insert("g").attr("class", "label branch-label");
    (q = p.node()) == null || q.appendChild(y);
    const w = y.getBBox();
    $ = te(l.name, $, f, w, e), p.remove(), g.remove(), d.remove();
