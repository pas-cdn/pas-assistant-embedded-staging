import { _ as n, s as gi, h as xi, J as Xt, I as di, i as pi, j as fi, m as Yt, a1 as yi, n as mi, P as bi, Z as Ct, Y as Nt, $ as Ai, a3 as wi, q as Ci, a4 as Si } from "./mermaid-VLURNSYL-C7dSNHug.js";
import { o as Bt, n as Wt, q as zt } from "./embed-entry-BQjeO8mI.js";
var mt = function() {
  var s = /* @__PURE__ */ n(function(W, r, u, g) {
    ...
    parse: /* @__PURE__ */ n(function(r) {
      var u = this, g = [0], x = [], A = [null], o = [], nt = this.table, p = "", lt = 0, Et = 0, hi = 2, It = 1, li = o.slice.call(arguments, 1), _ = Object.create(this.lexer), N = { yy: {} };
      for (var dt in this.yy)
        Object.prototype.hasOwnProperty.call(this.yy, dt) && (N.yy[dt] = this.yy[dt]);
      _.setInput(r, N.yy), N.yy.lexer = _, N.yy.parser = this, typeof _.yylloc > "u" && (_.yylloc = {});
      var pt = _.yylloc;
      o.push(pt);
...
Expecting ` + ut.join(", ") + ", got '" + (this.terminals_[T] || T) + "'" : yt = "Parse error on line " + (lt + 1) + ": Unexpected " + (T == It ? "end of input" : "'" + (this.terminals_[T] || T) + "'"), this.parseError(yt, {
        ...
        switch (E[0]) {
          case 1:
            ...
            break;
          case 2:
            if (O = this.productions_[E[1]][1], q.$ = A[A.length - O], q._$ = {
              ...
              return ft;
            O && (g = g.slice(0, -1 * O * 2), A = A.slice(0, -1 * O), o = o.slice(0, -1 * O)), g.push(this.productions_[E[1]][0]), A.push(q.$), o.push(q._$), Mt = nt[g[g.length - 2]][g[g.length - 1]], g.push(Mt);
            break;
          case 3:
            return !0;
        }
...
` + this.showPosition(), {
  ...
  return n(Y, "Parser"), Y.prototype = $, $.Parser = Y, new Y();
}();
mt.parser = mt;
var _i = mt;
function bt(s) {
...
var G, Ht = (G = class {
  ...
  getMaxDimension(t, i) {
    ...
    for (const c of t) {
      const d = Si(a, 1, c), m = d ? d.width : c.length * i, b = d ? d.height : i;
  ...
  constructor(t, i, e, a, c) {
    super(t, a, c, i), this.categories = e, this.scale = Bt().domain(this.categories).range(this.getRange());
  }
  setRange(t) {
    super.setRange(t);
  }
  ...
  constructor(t, i, e, a, c) {
    super(t, a, c, i), this.domain = e, this.scale = Wt().domain(this.domain).range(this.getRange());
  }
  getTickValues() {
...
}, n(at, "XYChartBuilder"), at), ot = 0, jt, rt = Tt(), ht = kt(), y = Rt(), wt = ht.plotColorPalette.split(",").map((s) => s.trim()), gt = !1, _t = !1;
function kt() {
  const s = wi(), t = Ct();
...
function ii(s) {
  const t = Math.min(...s), i = Math.max(...s), e = j(y.yAxis) ? y.yAxis.min : 1 / 0, a = j(y.yAxis) ? y.yAxis.max : -1 / 0;
  y.yAxis = {
    type: "linear",
    title: y.yAxis.title,
    min: Math.min(e, t),
...
function vt(s) {
  ...
  if (s.length === 0)
    return t;
  if (!gt) {
    const i = j(y.xAxis) ? y.xAxis.min : 1 / 0, e = j(y.xAxis) ? y.xAxis.max : -1 / 0;
    Dt(Math.min(i, 1), Math.max(e, s.length));
  }
  if (_t || ii(s), St(y.xAxis) && (t = y.xAxis.categories.map((i, e) => [i, s[e]])), j(y.xAxis)) {
...
function ai() {
  ...
  return y.title = Xt(), Ei.build(rt, y, ht, jt);
}
...
var Ii = /* @__PURE__ */ n(function() {
  bi(), ot = 0, rt = Tt(), y = Rt(), ht = kt(), wt = ht.plotColorPalette.split(",").map((s) => s.trim()), gt = !1, _t = !1;
}, "clear"), Vi = {
  getDrawableElem: ai,
  clear: Ii,
  setAccTitle: fi,
  ...
  n(I, "getTextTransformation"), Yt.debug(`Rendering xychart chart
` + s);
  const R = yi(t), L = R.append("g").attr("class", "main"), z = L.append("rect").attr("width", d.width).attr("height", d.height).attr("class", "background");
  mi(R, d.height, d.width, !0), R.attr("viewBox", `0 0 ${d.width} ${d.height}`), z.attr("fill", c.backgroundColor), a.setTmpSVGG(R.append("g").attr("class", "mermaid-tmp-group"));
        ...
        if (C.selectAll("rect").data(f.data).enter().append("rect").attr("x", (l) => l.x).attr("y", (l) => l.y).attr("width", (l) => l.width).attr("height", (l) => l.height).attr("fill", (l) => l.fill).attr("stroke", (l) => l.strokeFill).attr("stroke-width", (l) => l.strokeWidth), d.showDataLabel)
          if (d.chartOrientation === "horizontal") {
            ...
            let l = function(h, k, w) {
              const { data: S, label: $ } = h, Y = k * $.length * 0.7, W = S.x + S.width / 2, r = W - Y / 2, u = W + Y / 2, g = r >= S.x && u <= S.x + S.width, x = S.y + w + k <= S.y + S.height;
              return g && x;
            };
            n(l, "fitsInBar");
